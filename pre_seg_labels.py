...
def preprocess():
    ...
    for subject in range(len(data_clean)):
        ...
        for film in range(len(data_clean[subject])):
            ...
            # add label to film: (arousal valence) note: that valence and arousal were measured one per film
            film_label = np.array([ DREAMER_info["SubjectsData"][subject]["ScoreValence"][film],
                                   DREAMER_info["SubjectsData"][subject]["ScoreArousal"][film]])
